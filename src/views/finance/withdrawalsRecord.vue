...
<script>
  import { withdrawlog } from '@/api/finance'
  ...
  export default {
    ...
    methods: {
      ...
      request: function() {
        ...
        withdrawlog(this.pageNo, this.pageSize, this.beginDate, this.endDate, this.name,this.name1,this.name2,this.name3,this.name4).then(
          res => {
            if(res.code==200){
      ...
      exportTable() {
        withdrawlog(this.pageNo, this.pageSize1, this.beginDate, this.endDate, this.name,this.name1,this.name2,this.name3,this.name4).then(res => {
          if(res.code==200){
            this.allList = eval(res.data.list)
            this.total = res.data.total
            ...
            require.ensure([], () => {
              ...
              const tHeader = ['提现时间','流水号',  '会员名称','联系方式', '提现金额($)', '地址','交易状态',"备注",'审核时间'] //将对应的属性名转换成中文
              // const tHeader = [];
              const filterVal = ['create_time','merchant_order_id','username', 'phone', 'withdraw_amount', 'withdraw_token_id', 'order_status', 'remark','modify_time'] //table表格中对应的属性名
              const list = date //想要导出的数据
              const data = this.formatJson(filterVal, list)
              export_json_to_excel(tHeader, data, '提现记录列表excel')
            })
          }
        })
      }
